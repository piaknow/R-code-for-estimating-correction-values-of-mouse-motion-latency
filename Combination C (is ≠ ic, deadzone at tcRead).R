...
for(x in xsamples:1){ 			#initial condition of x_lastCount (range: 100~1 (1~0.01 count))
	...
	for(i in 1:(interval)){		#initial condition of t_sLastRead (range: 1~interval)
		...
		while(datapos <= datalength){					#scan till the end
			scanstick = data[datapos,2]				#slider's position
			currentpixels = ceiling((x * xgap - scanstick) / pixelgap)	#sum of sensor count
		...
		for(j in time:0){		#initial condition of t_cLastRead (range: -pinterval+1~0)
			...
			while(nowmouse <= minmouse + 1){	#scan to check the timing of reversal
				if(nowmouse < minmouse){			#update minimum
					minmouse = nowmouse
				}
